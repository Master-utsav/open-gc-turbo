...
export function PrivateRoomScreen({
  ...
  return (    
    <div className="fixed inset-0 z-30 pointer-events-none flex justify-center">
      <motion.div
        initial={{ x: "100%" }}
        animate={{ x: 0 }}
        exit={{ x: "100%" }}
        transition={{ type: "spring", stiffness: 280, damping: 30 }}
        className="pointer-events-auto w-full max-w-lg h-full border-x border-border/30 bg-background/95 backdrop-blur-md shadow-2xl relative overflow-hidden flex flex-col box-border font-sans"
      >
        ...
        {/* Private Room Ambient Glows */}
        <div className="absolute top-1/3 -right-36 w-72 h-72 rounded-full bg-primary/12 dark:bg-primary/6 blur-[80px] pointer-events-none z-0 transition-colors duration-500" />
        <div className="absolute bottom-1/3 -left-36 w-72 h-72 rounded-full bg-amber-500/12 dark:bg-amber-500/6 blur-[80px] pointer-events-none z-0 transition-colors duration-500" />
        ...
        <div className="relative z-10 flex flex-col h-full w-full overflow-hidden">
          ...
          {/* Messages */}
          <div className="flex-1 overflow-y-auto overflow-x-hidden py-4 flex flex-col gap-1.5 scroll-smooth">
            {messages.length === 0 && (
